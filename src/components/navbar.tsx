        ...
        <div className="flex justify-between items-center">
          ...
          <Link href="/" className="flex items-center space-x-3 group outline-none">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-xl tracking-tighter text-white leading-none uppercase">WebLicencia</span>
          ...
          <div className="hidden md:flex items-center bg-white/[0.03] border border-white/5 rounded-2xl px-2 py-1.5 backdrop-blur-md">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="group flex items-center gap-2 px-5 py-2 text-[10px] font-bold uppercase tracking-widest text-white/50 hover:text-white transition-all rounded-xl hover:bg-white/5"
              >
            ...
            {session ? (
              <div className="flex items-center space-x-2 bg-white/[0.03] border border-white/5 rounded-2xl p-1.5">
                {(session.user as any)?.role === 'ADMIN' && (
                  <Link href="/admin">
                    <Button size="sm" className="bg-accent text-black hover:bg-white font-bold uppercase tracking-widest text-[9px] px-4 h-9 rounded-xl shadow-[0_0_15px_rgba(var(--accent-rgb),0.2)]">
                      Portal Admin
                    </Button>
                  </Link>
                )}
              ...
              <div className="flex items-center gap-2">
                <Link href="/login" className="hidden sm:block">
                  <Button variant="ghost" className="text-[10px] font-bold uppercase tracking-widest text-white/40 hover:text-white rounded-xl h-11 px-6">
                    Acceso
                  </Button>
                </Link>
                <Link href="/register">
                  <Button className="bg-white text-black hover:bg-accent hover:text-white rounded-xl font-bold uppercase tracking-widest text-[10px] h-11 px-8 shadow-2xl transition-all border-none">
                    Unirse
                  </Button>
                </Link>
              </div>
          ...
          <div className="md:hidden absolute top-full left-4 right-4 bg-black/95 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-8 mt-4 space-y-6 animate-in fade-in zoom-in-95 duration-500 shadow-2xl overflow-hidden">
            ...
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="flex items-center gap-4 text-xl font-bold uppercase tracking-[0.2em] text-white/40 hover:text-accent transition-colors"
